...
2. Decide which path applies:
   - inspection only: stay with read-only `jj` commands, and load `references/jj-reference.md` only if you need targeted inspection helpers such as `jj show -r <rev>` or repo revsets
...
- `references/jj-reference.md` for quick-start inspection, shaping workflows, bookmark/push checklists, aliases, revsets, safety rules, recovery, and git-to-jj mapping
