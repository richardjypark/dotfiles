...
- `czl` is the Omarchy/Arch daily maintenance wrapper: it runs `czuf`, upgrades official Arch packages with `sudo pacman -Syu --noconfirm`, updates the globally installed Pi Coding Agent via npm when present, runs `chezmoi-bump --all`, then re-runs `chezmoi apply --refresh-externals --force` so the machine matches the bumped pins
